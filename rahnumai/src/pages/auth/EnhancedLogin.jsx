  ...
  const handleSubmit = async (e, card) => {
    ...
    try {
      if (card.title === "SignUp") {
        const res = await api.signUp(formValues.role, formValues);
        if (res.success) {
          alert(`Sign Up Successful! You can now login as ${formValues.role}`);
          setFormValues({});
          setShowSignUp(false);
          setFormErrors({});
        }
        ...
        let identifier;
        if (card.title === 'Student') identifier = formValues.enroll;
        if (card.title === 'Faculty') identifier = formValues.username;
        if (card.title === 'Admin') identifier = formValues.email;

        const res = await api.login(card.title.toLowerCase(), identifier, formValues.pass);
        if (res.success) {
          onLoginSuccess?.({
            role: card.title.toLowerCase(),
            name: formValues.name || identifier,
