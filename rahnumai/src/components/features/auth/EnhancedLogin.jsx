...
export default function EnhancedLogin({ onLoginSuccess }) {
  ...
  <div className={`relative min-h-screen overflow-hidden transition-colors duration-500 ${darkMode ? 'dark bg-gray-900' : 'bg-gray-50'}`}>
    ...
    <div className="relative z-10 flex flex-col items-center justify-center min-h-screen p-4 mobile-padding">
      {loading ? (
        <div className="flex flex-col items-center gap-6">
          <Skeleton width="48" height="12" className="mb-4" />
          <div className="flex flex-col md:flex-row gap-6 mobile-stack">
            {[1,2,3].map((item) => (
              <div key={item} className="w-full md:w-40 mobile-full">
        ...
        <>
          ...
          <div className={`flex flex-col md:flex-row gap-4 lg:gap-6 z-40 mobile-stack mobile-gap-4`}>
            {cardData.map((card) => {
              ...
              return (
                <div
                  ...
                  className={`transition-all duration-300 ease-in-out ${
                    isSelected
                      ? 'fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-50 w-11/12 max-w-md h-auto max-h-[90vh] overflow-y-auto'
                      : isOtherCardSelected
                        ? 'opacity-30 scale-50 pointer-events-none'
                        : 'relative cursor-pointer w-full md:w-48 lg:w-52 h-60 mobile-full'
                  }`}
                >
                  <MagicCard
                    spotlightColor={card.color}
                    darkMode={darkMode}
                    className={`flex items-center justify-center p-4 lg:p-6 overflow-hidden transition-transform duration-300 ease-in-out ${
                      isSelected ? 'scale-100 w-full h-full' : 'w-full h-full'
                    }`}
                    onClick={() => !isSelected && handleCardClick(card.title)}
                    disableHover={isSelected || isOtherCardSelected}
                  >
                    {isSelected ? (
                      <div className="flex flex-col justify-start gap-4 w-full max-w-md relative p-4">
                        <div
                          className="absolute -left-4 w-64 h-1 rounded-t mobile-hidden"
                          style={{
                            backgroundColor: card.color,
                            transform: `translateY(${card.title === "Student" ? "-115px" : "-153px"})`
                          }}
                        />
                        <div className="flex flex-col items-center mt-4 lg:mt-8 gap-4 px-2 lg:px-6">
                          <div className={`${darkMode ? 'text-white' : 'text-gray-900'} mb-4`}>
                            {React.cloneElement(card.icon, { size: 32 })}
                          </div>
                          <h2 className={`text-xl lg:text-2xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'} mobile-text-center`}>
                            {card.title} Login
                          </h2>
                        </div>
                        <form
                        ...
                        >
                          ...
                          <button
                            type="submit"
                            className="relative inline-block px-4 lg:px-6 py-3 mt-2 font-semibold text-white rounded-lg shadow-lg overflow-hidden transform transition-all duration-300 hover:scale-105 hover:shadow-2xl touch-button"
                      ...
                      <div className="flex flex-col items-center text-center gap-4 relative z-10 p-4">
                        <div className={`${darkMode ? 'text-white' : 'text-gray-900'} transition-transform duration-300 group-hover:scale-110`}>
                          {React.cloneElement(card.icon, { size: 32 })}
                        </div>
                        <h2 className={`text-lg lg:text-xl font-semibold ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                          {card.title}
                        </h2>
                        <p className={`text-xs lg:text-sm ${darkMode ? 'text-white/70' : 'text-gray-700'}`}>
                          {card.description}
                        </p>
                      </div>
                    )}
          ...
          {!selectedCard && !showSignUp && (
            <div className="mt-6 z-40">
              <span className="text-gray-500 dark:text-gray-400 text-sm">
                Not Registered Yet?{' '}
                <button
                  className="text-blue-500 font-semibold hover:underline transition-colors touch-button"
          ...
          {showSignUp && (
            <div className="fixed inset-0 flex items-center justify-center z-50 px-4 mobile-padding">
              <div
              ...
              <MagicCard
                ...
                darkMode={darkMode}
                className="relative w-full max-w-md h-auto max-h-[90vh] overflow-y-auto p-6 lg:p-8 scale-85 transition-transform duration-300 z-50"
              >
                <div
                ...
                <div className="flex flex-col items-center mt-4 gap-2">
                  <User size={32} className={darkMode ? 'text-white' : 'text-gray-900'} />
                  <h2 className={`text-xl lg:text-2xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'} mobile-text-center`}>
                    Sign Up
                  </h2>
                </div>
                ...
                >
                  ...
                  <button
                    type="submit"
                    className="relative inline-block px-4 lg:px-6 py-3 mt-2 font-semibold text-white rounded-lg shadow-lg overflow-hidden transform transition-all duration-300 bg-green-500 hover:scale-105 hover:shadow-2xl touch-button"
                  >
